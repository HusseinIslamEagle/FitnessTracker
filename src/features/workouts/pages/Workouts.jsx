      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
        {programs.map((program) => (
          <motion.div
            ...
            onClick={() =>
              setSelectedProgram(
                selectedProgram?.id === program.id ? null : program
              )
            }
          >
              ...
              {selectedProgram.days.map((day, index) => (
                ...
                >
                  <h3 className="text-xl font-bold mb-6 flex items-center gap-3">
                    <span className="w-8 h-8 rounded-lg bg-orange-500 flex items-center justify-center text-sm font-black italic">
                      D{index + 1}
                    </span>
                    {day.name}
                  </h3>
      ...
      <section className="relative mt-32 p-8 md:p-12 bg-white/[0.02] border border-white/5 rounded-[40px]">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto mt-4 text-xs uppercase tracking-widest">
            Live exercises from Wger API ({items.length}/{count || "—"}) • Showing{" "}
        ...
        {/* Loading */}
        {loading && (
          <div className="mt-6 text-center text-gray-400 text-sm">Loading...</div>
        )}
